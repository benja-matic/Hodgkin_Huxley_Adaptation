...
c += sum(W[0, 0:Ne_local]) - Aeex
c += sum(W[0, Ne_local:Ne2])
c += sum(W[0, Ne2:Ne2 + Ni_local]) - Aeix
c += sum(W[0, Ne2+Ni_local:N])

c += sum(W[Ne_local, 0:Ne_local])
c += sum(W[Ne_local, Ne_local:Ne2]) - Aeex
c += sum(W[Ne_local, Ne2:Ne2 + Ni_local])
c += sum(W[Ne_local, Ne2+Ni_local:N]) - Aeix

c += sum(W[Ne2, 0:Ne_local]) - Aiex
c += sum(W[Ne2, Ne_local:Ne2]) - AieNLx
c += sum(W[Ne2, Ne2:Ne2 + Ni_local]) - Aiix
c += sum(W[Ne2, Ne2+Ni_local:N])

c += sum(W[Ne2+Ni_local, 0:Ne_local]) - AieNLx
c += sum(W[Ne2+Ni_local, Ne_local:Ne2]) - Aiex
c += sum(W[Ne2+Ni_local, Ne2:Ne2 + Ni_local])
c += sum(W[Ne2+Ni_local, Ne2+Ni_local:N]) - Aiix
